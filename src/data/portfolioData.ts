export const links = {
  github: "https://github.com/mayurpatil70",
  linkedin: "https://www.linkedin.com/in/mayurpatil07/",
  instagram:
    "https://www.instagram.com/noball_ondesk?igsh=MWk5OHNmNXdwamN6OQ%3D%3D",
  email: "mayurthinks7@gmail.com",
  businessEmail: "noballondesk@gmail.com",
  phone: "+91 9689102662",
  resume: "/MayurPatil_CV.pdf",
  location: "Pune, Maharashtra",
};

export const heroData = {
  firstName: "Mayur",
  lastName: "Patil",
  title: "Full Stack Developer",
  description:
    "Passionate about building impactful products that solve real problems, scale efficiently, and perform reliably in production.",
  status: "Currently working at Cognizant as MEAN Stack Developer",
  about:
    "I am a dedicated Full Stack Developer with 1.4+ years of professional experience in designing, developing, and maintaining scalable enterprise applications using MongoDB, Express.js, Angular, Node.js, REST APIs and Agile methodologies. Currently working at Cognizant as a MEAN Stack Developer since January 2025. I have strong expertise in backend development, responsive UI design, API integration, debugging, and database optimization. I have successfully developed projects such as a secure banking application where I implemented advanced features like JWT authentication, AES-256 encryption, and multi-factor authentication, ensuring high-level security and reliability. My ability to understand business requirements, solve complex problems, and deliver projects on time makes me a reliable choice. I am committed to maintaining clear communication, ensuring quality, and providing solutions that truly add value to your project.",
};

export const experienceData = [
  {
    title: "Full Stack Developer",
    company: "Cognizant",
    siteurl: "https://www.cognizant.com",
    period: "Jan 2025 – Present",
    tech: [
      "MongoDB",
      "Express.js",
      "Angular",
      "Node.js",
      "REST APIs",
      "TypeScript",
      "JWT",
    ],
    highlights: [
      "Developed scalable enterprise-grade web applications using MEAN stack",
      "Designed responsive and reusable UI components using Angular, HTML5, CSS3, and TypeScript",
      "Built and maintained scalable REST APIs for enterprise applications",
      "Implemented authentication and authorization using JWT based security mechanism",
      "Optimized MongoDB queries and improved application performance for large datasets",
      "Collaborated with QA, UI/UX, and DevOps teams for smooth deployments",
    ],
    logo: "🏢",
  },
];

export const projectsData = [
  {
    title: "Retail Sales & Inventory Management System",
    subtitle: "Enterprise Retail Management Platform",
    description:
      "Enterprise retail management platform to handle sales transactions, inventory tracking, billing, supplier management, and executive dashboards across multiple store locations. Client: Global Retail Chain (USA).",
    icon: "🛍️",
    image: "/inventory-management.png",
    features: [
      "Scalable backend APIs for products, billing, sales and inventory modules",
      "Angular dashboards for stock monitoring, sales analytics, and KPI reporting",
      "Real-time stock updates across multiple warehouses",
      "Integrated barcode scanner and invoice generation modules",
      "Optimized MongoDB queries for high-volume transaction processing",
      "Improved system performance and reporting speed",
    ],
    tech: [
      "MongoDB",
      "Express.js",
      "Angular",
      "Node.js",
      "REST APIs",
      "TypeScript",
    ],
    liveDemo: "https://github.com/mayurpatil70",
    githubRepo: "https://github.com/mayurpatil70",
    period: "Aug 2025 – Present",
  },
  {
    title: "Secure Banking Application",
    subtitle: "Enterprise Banking Solution",
    description:
      "Enterprise-grade banking application with advanced security features including 2FA, JWT token rotation, and AES-256 encryption. Achieved 99.7% security rating with 40% faster login performance while supporting 1,000+ concurrent users.",
    icon: "🏦",
    image: "/secure-banking-app.png",
    features: [
      "Multi-factor authentication (2FA) increasing account security by 99.7%",
      "JWT token rotation system with 15-minute expiration reducing session hijacking risks by 90%",
      "AES-256-GCM encryption for 50,000+ sensitive records",
      "Optimized login processing time by 40% while supporting 1,000+ concurrent users",
      "Responsive frontend using React.js with seamless user authentication flow",
      "Google Authenticator integration",
    ],
    tech: [
      "Java",
      "Spring Boot",
      "React.js",
      "JavaScript",
      "MySQL",
      "Google Authenticator",
    ],
    liveDemo: "https://github.com/mayurpatil70",
    githubRepo: "https://github.com/mayurpatil70",
    period: "2024",
  },
];
