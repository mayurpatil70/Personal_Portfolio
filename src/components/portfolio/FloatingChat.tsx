...
import {
  MessageCircle, X, Send, ChevronRight, Loader2,
  Clock, CheckCheck, Plus, ArrowLeft, Inbox, Trash2, Sparkles,
} from "lucide-react";
...
const categories = [
  { value: "idea",    label: "Discuss an Idea", emoji: "💡", color: "#f59e0b", bg: "rgba(245,158,11,0.12)",  description: "Let's brainstorm together" },
  { value: "hiring",  label: "Hiring / HR",      emoji: "💼", color: "#6366f1", bg: "rgba(99,102,241,0.12)", description: "Looking for talent" },
  { value: "project", label: "Collaboration",    emoji: "🚀", color: "#10b981", bg: "rgba(16,185,129,0.12)", description: "Build something cool" },
  { value: "general", label: "Just Chatting",    emoji: "💬", color: "#3b82f6", bg: "rgba(59,130,246,0.12)", description: "Say hello" },
  { value: "meeting", label: "Book a Meeting",   emoji: "📅", color: "#ec4899", bg: "rgba(236,72,153,0.12)", description: "Schedule time together" },
];

const categoryFields: Record<string, { id: string; label: string; placeholder: string; type?: string; multiline?: boolean }[]> = {
  idea: [
    { id: "name",   label: "Your Name",       placeholder: "Ada Lovelace" },
    { id: "email",  label: "Email",           placeholder: "ada@example.com", type: "email" },
    { id: "idea",   label: "The Idea",        placeholder: "Describe your idea in a few lines…", multiline: true },
    { id: "collab", label: "How can I help?", placeholder: "Feedback, co-build, mentorship…", multiline: true },
  ],
  hiring: [
    { id: "name",    label: "Your Name",       placeholder: "Recruiter's name" },
    { id: "email",   label: "Work Email",      placeholder: "hr@company.com", type: "email" },
    { id: "company", label: "Company / Org",   placeholder: "Acme Inc." },
    { id: "role",    label: "Role / Position", placeholder: "Frontend Developer, Full-stack…" },
    { id: "message", label: "Tell me more",    placeholder: "Compensation, timeline, expectations…", multiline: true },
  ],
  project: [
    { id: "name",    label: "Your Name",              placeholder: "Your name" },
    { id: "email",   label: "Email",                  placeholder: "you@example.com", type: "email" },
    { id: "project", label: "Project Name / Summary", placeholder: "What are we building?" },
    { id: "stack",   label: "Tech Stack",             placeholder: "React, Node, Python…" },
    { id: "message", label: "Details & Expectations", placeholder: "Timeline, scope, your role…", multiline: true },
  ],
  general: [
    { id: "name",    label: "Your Name", placeholder: "Hey, I'm…" },
    { id: "email",   label: "Email",    placeholder: "so I can reply 😊", type: "email" },
    { id: "message", label: "Message",  placeholder: "Anything on your mind!", multiline: true },
  ],
  meeting: [
    { id: "name",         label: "Your Name",        placeholder: "Your name" },
    { id: "email",        label: "Email",            placeholder: "you@example.com", type: "email" },
    { id: "agenda",       label: "Meeting Agenda",   placeholder: "What should we discuss?" },
    { id: "availability", label: "Your Availability",placeholder: "Mon–Fri after 3pm IST, or share a Calendly…" },
  ],
};
...
function loadConvos(): Conversation[] {
  try { return JSON.parse(localStorage.getItem(LS_CONVOS) ?? "[]"); } catch { return []; }
}
function saveConvos(convos: Conversation[]) {
  try { localStorage.setItem(LS_CONVOS, JSON.stringify(convos)); } catch {}
}
...
function stampRL() {
  try { localStorage.setItem(LS_RL, Date.now().toString()); } catch {}
}

// ── Misc helpers ──────────────────────────────────────────────────────────────
function uid() { return Math.random().toString(36).slice(2, 10); }

function buildFormSummary(category: string, data: Record<string, string>): string {
...
async function fetchThankYou(category: string, data: Record<string, string>): Promise<string> {
  const res = await fetch(`${WORKER_URL}/thankyou`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ category, name: data.name || "there", formData: data }),
  });
  if (!res.ok) throw new Error(await res.text());
...
const BubbleList = ({ bubbles, endRef }: { bubbles: Bubble[]; endRef?: React.RefObject<HTMLDivElement> }) => (
          ...
          {bubble.role === "system" && (
            <div className="flex justify-center">
              <span className="text-xs text-muted-foreground bg-muted/60 px-3 py-1 rounded-full flex items-center gap-1.5">
                <CheckCheck className="h-3 w-3 text-green-500" /> {bubble.content}
              </span>
            </div>
          )}
          ...
          {bubble.role === "user" && (
            <div className="flex justify-end">
              <div className="max-w-[85%]">
                {bubble.label && (
                  <p className="text-xs text-right text-muted-foreground mb-1 pr-1 opacity-70">{bubble.label}</p>
                )}
                <div
                  className="rounded-2xl rounded-tr-sm px-4 py-2.5 text-white text-sm leading-relaxed"
                  style={{ background: `linear-gradient(135deg, hsl(var(--theme-primary)), hsl(var(--theme-secondary)))` }}
                >
                  {bubble.content.split("\n").map((line, li) => {
                    const field = line.match(/^\*\*(.+?):\*\* (.+)$/);
                    if (field) return (
                      <div key={li} className="mb-0.5">
                        <span className="opacity-60 text-xs">{field[1]}: </span>
                        <span>{field[2]}</span>
                      </div>
                    );
                    const hdr = line.match(/^(.+?) \*\*(.+)\*\*$/);
                    if (hdr) return <div key={li} className="font-semibold mb-1.5">{hdr[1]} {hdr[2]}</div>;
...
export const FloatingChat = () => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("floatingchat:open-contact", handleOpenContact);
  }, []);

  // ── Rotate floating label ───────────────────────────────────────────────────
  useEffect(() => {
    const iv = setInterval(() => setMsgIdx((p) => (p + 1) % floatingMessages.length), 3000);
  ...
  const updateConvo = (id: string, updater: (c: Conversation) => Conversation) => {
    setConvos((prev) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create the conversation with user bubble
    ...
    const [, tyResult] = await Promise.allSettled([
      postToDiscord(selectedCat, formData).catch(console.error),
      fetchThankYou(selectedCat, formData).catch(
        () => `Thanks ${formData.name || ""}! I'll be in touch soon. 🚀`
      ),
    ]);

    const msg = tyResult.status === "fulfilled"
      ? (tyResult.value as string)
      : `Thanks ${formData.name || ""}! I'll be in touch soon. 🚀`;

    // Replace typing indicator with real AI reply + system notice
    setConvos((prev) => {
      const next = prev.map((c) => {
        ...
        return {
          ...
          bubbles: [
            ...
            { role: "system" as BubbleRole, content: "✅ Sent to inbox — I'll reply to your email shortly!" },
          ],
        };
      });
      saveConvos(next);
  ...
  const headerInfo = (): { title: string; subtitle: string } => {
    if (view === "home")    return { title: "Messages",              subtitle: convos.length > 0 ? `${convos.length} conversation${convos.length > 1 ? "s" : ""}` : "No messages yet" };
    if (view === "select")  return { title: "New Message",           subtitle: "Pick a reason to connect" };
    if (view === "form")    return { title: `${activeCatMeta?.emoji} ${activeCatMeta?.label}`, subtitle: activeCatMeta?.description ?? "" };
    if (view === "chat")    return { title: activeConvo ? (categories.find((c) => c.value === activeConvo.category)?.label ?? "Chat") : "Chat", subtitle: activeConvo ? formatRelTime(activeConvo.timestamp) : "" };
  ...
  const handleBack = () => {
    ...
    else if (view === "select" || view === "chat") setView(convos.length > 0 ? "home" : "select");
  };

  const hi = headerInfo();

  // ── Unread badge: convos with loading bubbles (submitting) ───────────────────
  const submittingCount = convos.filter((c) => c.bubbles.some((b) => b.loading)).length;
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            >
              {canGoBack && (
                <button onClick={handleBack} className="text-white/80 hover:text-white p-1 rounded-full transition-colors flex-shrink-0">
                  <ArrowLeft className="h-4 w-4" />
                </button>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-white font-semibold text-sm leading-tight truncate">{hi.title}</p>
                <p className="text-white/65 text-xs mt-0.5 truncate">{hi.subtitle}</p>
              </div>
              ...
              <button onClick={handleClose} className="text-white/80 hover:text-white p-1 rounded-full transition-colors flex-shrink-0">
                <X className="h-4 w-4" />
              </button>
            </div>
                  ...
                  >
                    ...
                    <div className="px-4 pt-4 pb-2">
                      ...
                      >
                        ...
                        {cooldown > 0 ? (
                          <><Clock className="h-4 w-4" /> New message in {cooldown}s</>
                        ) : (
                    ...
                    {convos.length === 0 ? (
                      <div className="flex-1 flex flex-col items-center justify-center gap-3 p-8 text-center">
                        ...
                        <p className="text-xs text-muted-foreground">Start a conversation and it'll appear here.</p>
                      </div>
                    ) : (
                      <div className="flex flex-col divide-y" style={{ borderColor: "hsl(var(--border))" }}>
                        {convos.map((convo, i) => {
                          const cat      = categories.find((c) => c.value === convo.category);
                          const lastBubble = [...convo.bubbles].reverse().find((b) => !b.loading && b.role !== "system");
                          const isLoading  = convo.bubbles.some((b) => b.loading);
                          const preview    = lastBubble?.content.replace(/\*\*/g, "").split("\n")[0] ?? "";
                          return (
                            <motion.button
                              key={convo.id}
                            ...
                            >
                              ...
                              <div
                                ...
                                style={{ background: cat ? cat.bg : "rgba(0,0,0,0.05)", border: `1.5px solid ${cat?.color ?? "#ccc"}30` }}
                              >
                                {cat?.emoji ?? "💬"}
                              </div>
                              {/* Text */}
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-1">
                                  <p className="text-sm font-semibold truncate" style={{ color: cat?.color }}>{cat?.label ?? convo.category}</p>
                                  <span className="text-xs text-muted-foreground flex-shrink-0">{formatRelTime(convo.timestamp)}</span>
                                </div>
                    ...
                    {/* Footer */}
                    <div className="mt-auto px-4 py-3 border-t flex items-center justify-between" style={{ borderColor: "hsl(var(--border))" }}>
                      <p className="text-xs text-muted-foreground">Stored locally on your device</p>
                      {convos.length > 0 && (
                        <button
                          onClick={() => { setConvos([]); saveConvos([]); }}
                  ...
                  >
                    {cooldown > 0 && (
                      ...
                      >
                        ...
                        Rate limited — new submission in <strong className="ml-1">{cooldown}s</strong>
                      </motion.div>
                    )}
                    {categories.map((cat, i) => (
                      ...
                      >
                        ...
                        <ChevronRight className="h-4 w-4 flex-shrink-0 transition-transform group-hover:translate-x-1" style={{ color: cat.color }} />
                      </motion.button>
                    ))}
                  </motion.div>
                )}
                  ...
                  >
                    {fields.map((field) => (
                      <div key={field.id} className="flex flex-col gap-1">
                        ...
                        {field.multiline ? (
                          <textarea required rows={3} placeholder={field.placeholder}
                          ...
                          <input required type={field.type || "text"} placeholder={field.placeholder}
                            value={formData[field.id] || ""}
                            onChange={(e) => setFormData((p) => ({ ...p, [field.id]: e.target.value }))}
                    ...
                    <motion.button type="submit" disabled={isSubmitting || cooldown > 0}
                      whileTap={{ scale: 0.97 }}
                  ...
                  >
                    ...
                    <div className="px-4 pt-3 pb-1 flex justify-center">
                      <span className="text-xs text-muted-foreground bg-muted/50 px-3 py-1 rounded-full">
                        {new Date(activeConvo.timestamp).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}
                      </span>
                    </div>

                    {/* Bubbles */}
                    <div className="flex-1 overflow-y-auto px-4 pb-4 pt-2">
                      <BubbleList bubbles={activeConvo.bubbles} endRef={chatEndRef} />
                    </div>

                    {/* Footer */}
                    <div className="flex-shrink-0 border-t px-4 py-2.5" style={{ borderColor: "hsl(var(--border))" }}>
                      {chatLocked ? (
                        <div className="flex flex-col gap-1">
                          ...
                          >
                            {cooldown > 0
                              ? <><Clock className="h-3.5 w-3.5" /> New message in {cooldown}s</>
                              : <><Plus className="h-3.5 w-3.5" /> Start a new message</>
                            }
                          </button>
                        </div>
                      ) : (
      ...
      <div className="flex items-center gap-3">
          ...
          {showPeek && !isOpen && (
            ...
            >
              ...
              <div
                ...
                onClick={() => { setShowPeek(false); setPeekDismissed(true); handleOpen(); }}
              >
                {/* Dismiss X */}
                <button
                  className="absolute top-2 right-2 text-muted-foreground hover:text-foreground transition-colors"
                  onClick={(e) => { e.stopPropagation(); setShowPeek(false); setPeekDismissed(true); }}
                >
                  <X className="h-3 w-3" />
                </button>
        ...
        <motion.button
          ...
          animate={showPeek && !isOpen
            ? { rotate: [0, -12, 12, -8, 8, -4, 4, 0], scale: [1, 1.08, 1.08, 1.08, 1.08, 1.05, 1.05, 1] }
            : {}
          }
          transition={showPeek ? { duration: 0.8, delay: 0.3, ease: "easeInOut" } : {}}
        >
          <AnimatePresence mode="wait">
            {isOpen ? (
          ...
          {!isOpen && convos.length === 0 && (
            <>
              ...
              <motion.span
                ...
                transition={{ duration: 1.5, repeat: showPeek ? Infinity : 0, repeatDelay: 0.5 }}
              />
            </>
          )}
        </motion.button>
